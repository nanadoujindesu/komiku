...
/**
 * WordPress Photon CDN endpoints — they act as a public image proxy
 * and can bypass Cloudflare IP blocks since they use whitelisted IPs.
 */
const PHOTON_HOSTS = ['i0.wp.com', 'i1.wp.com', 'i2.wp.com', 'i3.wp.com'];
let photonIndex = 0;
function nextPhotonHost() {
  const host = PHOTON_HOSTS[photonIndex % PHOTON_HOSTS.length];
  photonIndex++;
  return host;
}

/**
 * Build a Photon CDN URL for an image.
 * Photon format: https://i0.wp.com/original-host/path
 */
function toPhotonUrl(originalUrl) {
  try {
    const parsed = new URL(originalUrl);
    const photonHost = nextPhotonHost();
    // Photon expects: https://i0.wp.com/HOSTNAME/PATH?query
    return `https://${photonHost}/${parsed.hostname}${parsed.pathname}${parsed.search}`;
  } catch {
    return null;
  }
}
 ...
 * Strategy:
 *   1. Direct fetch with smart Referer
 *   2. If 403 → retry with fallback Referers
 *   3. If still 403 → proxy through WordPress Photon CDN (i0-i3.wp.com)
 *   4. If still 403 → try with no Referer
 */
async function fetchImage(url) {
  return deduplicated(`img:${url}`, async () => {
    ...
    // Proxy through WordPress Photon CDN (bypasses Cloudflare IP blocks)
    const photonUrl = toPhotonUrl(url);
    if (photonUrl) {
      console.log(`[img] All direct attempts 403. Trying Photon CDN: ${photonUrl}`);
      try {
        const photonResp = await axios({
          method: 'GET',
          url: photonUrl,
          headers: {
            'User-Agent': ua,
        ...
        if (photonResp.status === 200) {
          console.log(`[img] Photon CDN success for ${url}`);
          return photonResp;
        }
        // Try another Photon host if first one failed
        const photonUrl2 = toPhotonUrl(url);
        const photonResp2 = await axios({
          method: 'GET',
          url: photonUrl2,
          headers: {
            'User-Agent': ua,
        ...
        if (photonResp2.status === 200) {
          console.log(`[img] Photon CDN (2nd host) success for ${url}`);
          return photonResp2;
        }
      } catch (e) {
        console.warn(`[img] Photon CDN failed for ${url}:`, e.message);
      }
    }
